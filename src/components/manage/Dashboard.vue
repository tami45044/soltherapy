<template>
  <v-container dir="rtl" class="dashboard-container">
    <!-- Header -->
    <v-row class="mb-8">
      ...
      <v-col cols="12" sm="6" lg="3">
        <v-card class="stat-card-modern" rounded="xl" elevation="0" hover>
          ...
          <v-card-text class="pa-5 position-relative">
            <div class="d-flex justify-space-between align-center">
              <div class="text-right">
                <div class="text-h3 font-weight-bold info--text mb-1">₪{{ stats.weekRevenue.toLocaleString() }}</div>
                <div class="text-body-2 text-medium-emphasis">הכנסות השבוע</div>
              </div>
      ...
      <v-col cols="12" sm="6" lg="3">
        <v-card class="stat-card-modern" rounded="xl" elevation="0" hover>
          ...
          <v-card-text class="pa-5 position-relative">
            <div class="d-flex justify-space-between align-center">
              <div class="text-right">
                <div class="text-h3 font-weight-bold deep-purple--text mb-1">{{ stats.totalClients }}</div>
                <div class="text-body-2 text-medium-emphasis">סה"כ לקוחות</div>
              </div>
    ...
    <v-row class="mb-6">
      <v-col cols="12" lg="7">
        <v-card rounded="xl" elevation="2" dir="rtl">
          <v-card-title class="pa-6 bg-grey-lighten-4 text-right">
            פגישות היום
            <v-icon icon="mdi-calendar-today" class="me-2" />
          </v-card-title>

          <v-card-text class="pa-0">
            <v-list v-if="todayAppointments.length > 0" dir="rtl">
              <v-list-item
      ...
      <!-- Debts Summary -->
      <v-col cols="12" lg="5">
        <v-card rounded="xl" elevation="3" dir="rtl">
          <v-card-title class="pa-6 bg-error-lighten-4 text-right">
            חובות ללקוחות
            <v-icon icon="mdi-alert-circle" class="me-2" />
          </v-card-title>

          <v-card-text class="pa-0">
            <v-list v-if="clientsWithDebts.length > 0" dir="rtl">
              <v-list-item
        ...
        <!-- Quick Actions -->
        <v-card rounded="xl" elevation="3" class="mt-4" dir="rtl">
          <v-card-title class="pa-6 bg-grey-lighten-4 text-right">
            פעולות מהירות
        ...
        <v-card rounded="xl" elevation="3" dir="rtl">
          ...
          <v-card-text class="pa-6">
            <v-row>
              <v-col cols="12" md="4">
                <div class="stat-box">
                  <div class="stat-label">פגישות החודש</div>
                  <div class="stat-value">{{ stats.monthAppointments }}</div>
                </div>
              </v-col>
              ...
              <v-col cols="12" md="4">
                <div class="stat-box">
                  ...
                  <div class="stat-value text-success">₪{{ stats.monthRevenue }}</div>
                </div>
              </v-col>
              ...
              <v-col cols="12" md="4">
                <div class="stat-box">
                  ...
                  <div class="stat-value text-info">{{ stats.attendanceRate }}%</div>
                </div>
              </v-col>
            </v-row>
          </v-card-text>
...
const stats = computed(() => {
  ...
  const weekRevenue = weekAppts
    .filter(a => a.paid)
    .reduce((sum, a) => sum + (a.paymentAmount || a.price), 0)
